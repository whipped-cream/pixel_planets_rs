...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let fbm1 = fbm(rotated_uv, u_params.octaves, u_params.seed);
    d_light += fbm(rotated_uv * u_params.size + fbm1 + vec2f(globals.time * u_params.time_speed, 0.), u_params.octaves, u_params.seed) * 0.3; // change the magic 0.3 here for different light strengths
