...
@group(#{MATERIAL_BIND_GROUP}) @binding(5) var<uniform> u_light_border_1: f32;
@group(#{MATERIAL_BIND_GROUP}) @binding(6) var<uniform> u_light_border_2: f32;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    d_light += fbm(rotated_uv * u_size + vec2f(globals.time * u_time_speed, 0.), u_octaves, u_seed) * 0.3; // change the magic 0.3 here for different light strengths
